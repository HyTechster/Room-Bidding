...
| 2026-07-29 | **UX improvements: frozen prices, currency, tap-to-place, no em dashes** | (1) Prices are frozen while placing people and only recalculate on Continue (stops the confusing live price jumps). (2) Currency selector (`App\Support\Currency`, RM default) applied across the tool, result page and dashboard. (3) Tap-to-place (select a name, tap a room) added alongside drag, so the board works on touch/mobile; layout made responsive. (4) Em dashes removed from all user-facing text. |

| 2026-07-30 | **Adopted the JSX design (blue theme), adapted to single-operator** | Converted `design/*.jsx` visuals into Blade: results page as a "receipt" (dark header, balance proof, desktop table + mobile cards, collapsible print-open audit log), and the tool's setup + bidding steps (rounded-xl cards, status badges with icons, scope banner, sticky Next/Finish footer). Dropped the multi-user-only pieces the designs implied (join lobby, per-user lock/confirm, anonymity, host/member). App primary colour swept indigo → blue. |
